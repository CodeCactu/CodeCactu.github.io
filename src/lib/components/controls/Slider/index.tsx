...
import { SliderContextValue } from "./SliderContext"

export type SliderProps = SliderContextValue & {
  children: ReactNode
  className?: string
...
export default function Slider({ children, className, reachedEnds, maxVisibleCount, noloop, scroll, setReachedEnds, scrollableAreaRef, scrollableAreaGapRef }:SliderProps) {
  ...
  useEffect( () => { // Init
    ...
    if (noloop) {
      if (scrollArea.offsetWidth === scrollArea.scrollWidth && (!reachedEnds.left || !reachedEnds.right)) {
