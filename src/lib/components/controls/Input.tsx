...
export default function Input({ className, placeholderClassName, style, value:externalValue, type = `text`, name, disabled, readOnly, autoComplete, placeholder, maxLength, minLength, rows, onBlur, onChange, onCopy, validate }:InputProps) {
...
function getStylesWithPredicate( props:CSSClassesValues, predicate:(k:string, v:unknown) => boolean ) {
  return Object.fromEntries( Object.entries( props ).filter( ([ k, v ]) => predicate( k, v ) ) )
}
...
const useStyles = createStylesHook( ({ components }) => ({
  ...
  placeholder: {
    ...
    inset: 0,
    margin: 0,
    paddingInline: components.Input?.padding ?? 0,
